...
* **Known limitations (do not ship as-is):**
  ...
  * **No room access control.** Knowing a 16-bit conversation ID is enough to send `KEY_REQ`. Pinning authenticates the *server*, not members. `senderId` is client-chosen; the server only refuses to bind one id to two different public keys.
