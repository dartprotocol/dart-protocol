...
class NativeDartClient {
    ...
    private syncProbe(convId: number) {
        this.syncTimers.push(setTimeout(() => {
            ...
            // Re-arm while the latest message stays unacknowledged so
            // SYNC-driven NACK repair keeps retrying under loss.
    ...
    private handleMessage(buf: Buffer) {
        try {
            ...
            if (type === TYPE_DATA) {
                ...
                // decrypt my stream without one). Re-share the current chain
                // state: it's deterministic, so receiving it is always safe.
                ...
                if (seqDelta(this.highestSentSeq, ack.seq) < SEQ_MOD / 2) {
                    // Record the ACK; the re-arming sync probe stops on its
                    // own once the latest message is acknowledged.
